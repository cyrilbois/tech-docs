---
title:  三步让Spring Boot拥有缓存的能力
description: 三步让Spring Boot拥有缓存的能力，包括页面级别的缓存实现
...
...
页面级别的缓存通过Filter在请求到Controller之前就返回相关结果。 通过访问`http://localhost:9099/cache` 可以测试页面缓存的效果。
